 ...
 */
@Composable
fun ScreenInfoScreen(viewModel: ScreenInfoViewModel = viewModel { ScreenInfoViewModel() }) {
    ...
    Column(modifier = Modifier.padding(6.dp)) {
        InfoCard(title = "屏幕分辨率") {
            ...
            ResolutionInputItem(
                ...
                onModifyWidthInput = { viewModel.modifyWidthInput(it) },
                onModifyHeightInput = { viewModel.modifyHeightInput(it) },
                onModifyResolution = { viewModel.modifyResolution() }
            )
        }
        ...
        InfoCard(title = "屏幕密度") {
            ...
            ) { viewModel.resetDensity() }

            // 修改区域
            DensityInputItem(
                value = uiState.inputDensity,
                onModifyInput = { viewModel.modifyDensityInput(it) },
                onModify = { viewModel.modifyDensity() }
            )
        }
    }
}
