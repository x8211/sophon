...
fun ThreadScreen(viewModel: ThreadViewModel = viewModel { ThreadViewModel() }) {
    val threads by viewModel.uiState.collectAsState()
    Column {
        summary(threads)
...
private fun tableScreen(data: List<ThreadInfo>) {
    LazyColumn(modifier = Modifier.fillMaxSize()) {
        items(data) { threadInfo ->
            val user = threadInfo.user
            val pid = threadInfo.pid
            val tid = threadInfo.tid
            val ppid = threadInfo.ppid
            val vsz = threadInfo.vsz
            val rss = threadInfo.rss
            val wchan = threadInfo.wchan
            val address = threadInfo.address
            val state = threadInfo.state
            val cmd = threadInfo.cmd
